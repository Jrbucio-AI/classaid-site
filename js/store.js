...
const Store = (function(){
  const ROOT_KEY = 'classaid-v3';
  // root = { active: 'default', profiles: { id: { data:{classes,assignments,decks}, settings:{theme,accent,layout,modules,shortcuts,focus,navOrder,savedViews} } } }
  function root(){
  ...
  function ensureProfile(id){
    ...
    if(!r.profiles[id]){
      r.profiles[id] = {
        data: { classes:[], assignments:[], decks:[] },
        settings: {
  ...
  function ctx(){
    const r = ensureProfile(activeId());
    return { r, id: r.active, prof: r.profiles[r.active] };
  }
  return {
    // profile management
    ...
    rename(oldId, newId){
      const r = root(); if(!r.profiles[oldId]) return;
      r.profiles[newId] = r.profiles[oldId]; delete r.profiles[oldId];
      if(r.active===oldId) r.active = newId; save(r);
    },
    remove(id){
      const r = root(); delete r.profiles[id];
      if(r.active===id) r.active = Object.keys(r.profiles)[0] || 'default'; save(r);
    },
    // data (per-profile)
