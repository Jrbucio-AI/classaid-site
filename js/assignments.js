  ...
  function render(){
    ...
    list.forEach((a, idx)=>{
      ...
      tr.innerHTML = `<td>${a.title}</td>
        ...
        <td>${a.status==='done'?'<span class=\"badge done\">Done</span>': (soon?'<span class=\"badge soon\">Soon</span>':'<span class=\"badge\">Open</span>')}</td>
    ...
    const sv = document.getElementById('btnSaveView'); if(sv) sv.onclick = ()=>{
    ...
    const app = document.getElementById('btnApplyFilter'); if(app) app.onclick = ()=> render();
