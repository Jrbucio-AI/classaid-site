...
applyTheme();
const sFont=document.getElementById('sFont'), sLh=document.getElementById('sLh'), sAccent=document.getElementById('sAccent'), sContrast=document.getElementById('sContrast');
if(sFont){ sFont.value=settings.fontScale; sLh.value=settings.lh; sAccent.value=settings.accent; sContrast.checked=!!settings.contrast; }
[sFont,sLh,sAccent,sContrast].forEach(el=>{
  if(!el) return;
...
const toast = m => {const t=document.getElementById('toast'); if(!t) return; t.textContent=m; t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),2000)};
...
document.getElementById('addClass').onclick=()=>{
  ...
  if(!n) return; classes.push({n,cr,color}); renderClasses(); document.getElementById('clsName').value=''; document.getElementById('clsCred').value='';
};
renderClasses();
...
// ---------- Study sets ----------
function renderSets(){
  const setList=document.getElementById('setList'), playSet=document.getElementById('playSet');
  setList.innerHTML = sets.map((s,i)=>`<li><b style="color:${s.color}">${s.name}</b> — ${s.items.length} terms <button class='btn alt' data-i='${i}' onclick='window.__editSet(${i})' style='padding:4px 8px;margin-left:6px'>Edit</button></li>`).join('');
  playSet.innerHTML = sets.map((s,i)=>`<option value="${i}">${s.name}</option>`).join('');
  db.set('sets', sets);
}
window.__editSet = (i)=>{
...
document.getElementById('addSet').onclick=()=>{
  const name=document.getElementById('setName').value.trim(); if(!name) return;
  const color=document.getElementById('setColor').value||'#35c995'; sets.push({name,color,items:[]}); renderSets(); document.getElementById('setName').value='';
};
...
document.getElementById('pFlash').onclick=()=>{
  const s=sets[+document.getElementById('playSet').value]; if(!s||s.items.length===0){ toast('Add terms first'); return; }
...
document.getElementById('pMatch').onclick=()=>{
  const s=sets[+document.getElementById('playSet').value]; if(!s||s.items.length<4){ toast('Need at least 4 terms'); return; }
    ...
    tile.onclick=()=>{
      ...
      if(!first){ first=tile; }
        ...
        if(first.getAttribute('data-k')===tile.getAttribute('data-k') && first!==tile){
          ...
          const a=first; setTimeout(()=>{ a.style.borderColor=''; tile.style.borderColor=''; },280);
        }
        first=null;
      }
    };
...
document.getElementById('pMCQ').onclick=()=>{
  ...
  let q=0,score=0,limit=Math.min(10,items.length);
  els.playArea.innerHTML='';
  ...
  function draw(){
    ...
    choices.querySelectorAll('.choice').forEach(c=>c.onclick=()=>{
      ...
      setTimeout(()=>{ q++; if(q>=limit){ meta.textContent='Score: '+score+'/'+limit; toast('MCQ finished: '+score+'/'+limit); } else draw(); },420);
    });
    meta.textContent='Question '+(q+1)+'/'+limit+' — Score '+score;
  }
  draw();
};
...
// ---------- Export / Import ----------
function exportAll(){
  const data={classes,grades,tasks,quicknotes:notes,settings,sets,version:6};
...
// ---------- Overlay toggle chip ----------
setTimeout(()=>{ const mini=document.getElementById('caMini'); mini.style.display='inline-flex'; mini.onclick=()=>window.ClassAidOverlay&&window.ClassAidOverlay.toggle(); },800);
