...
(function(){
  ...
  function ensure(){
    ...
    // Append to the top of the DOM to dodge stacking contexts
    (document.scrollingElement||document.documentElement).appendChild(root);

    const bar=document.createElement('div');
    bar.style.cssText='position:fixed;right:16px;top:16px;background:#0d1311;border:1px solid #1b2622;color:#e8f1ed;padding:8px 10px;border-radius:12px;display:flex;gap:8px;align-items:center;pointer-events:auto;z-index:'+ (zBase+100);
    ...
    function make(n){
      ...
      Object.assign(el.style,{position:'fixed',left:(n.x||60)+'px',top:(n.y||80)+'px',width:'240px',background:'#fff8c6',color:'#1a1a1a',border:'1px solid #d8c96a',borderRadius:'8px',boxShadow:'0 10px 30px rgba(0,0,0,.35)',padding:'8px',pointerEvents:'auto',zIndex: (++zBase) });
      el.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><input class="tt" style="border:none;background:transparent;font-weight:700;width:160px" value="'+(n.t||'Note')+'"><div><button class="pin" style="border:none;background:transparent;margin-right:6px">📌</button><button class="x" style="border:none;background:transparent;font-weight:700">×</button></div></div><textarea class="bd" style="width:100%;height:140px;border:none;background:transparent">'+(n.b||'')+'</textarea>';
      root.appendChild(el);
      let drag=false,dx=0,dy=0;
      const header=el.querySelector('.tt');
      header.addEventListener('mousedown',e=>{drag=true;el.style.zIndex=++zBase;dx=e.clientX-parseInt(el.style.left);dy=e.clientY-parseInt(el.style.top);document.body.style.userSelect='none'});
      ...
      el.querySelector('.pin').onclick=()=>{ el.style.zIndex=++zBase; };
    ...
    function save(){
      state.notes = Array.from(root.querySelectorAll('textarea.bd')).map(ta=>{ const el=ta.closest('div'); return {x:parseInt(el.style.left), y:parseInt(el.style.top), t:el.querySelector('.tt').value, b:ta.value}; });
      write(state);
    }
  ...
  function toggle(){ const r=ensure().root; r.style.display=(r.style.display==='none')?'block':'none'; }
